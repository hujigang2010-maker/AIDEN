...
def main() -> None:
    ...
    # P13 收入结构
    s = prs.slides.add_slide(blank)
    add_title_bar(s, "P13 收入结构（与 Excel 测算一一对应）")
    add_table(s, 1.3,
        ["收入类别", "测算逻辑", "结算节奏"],
        [
            ["固定月费 Retainer", "30–50 万元/月", "按月支付"],
            ["专项奖项激励", "牌照 / 福布斯 / 小镇奖项 → 按数量奖励", "按事件结算"],
            ["活动运营收入", "单场科技开放麦 / 潮玩大赛执行利润分成", "按场结算"],
            ["招商佣金", "实际成交年租金的 1–2.5 个月（按面积阶梯）", "签约 + 起租后"],
            ["股权分红", "合资公司净利润按股比分配", "年度结算"],
        ])
    add_footer(s)
    ...
    add_table(s, 1.3,
        ["场景", "月费 (年化)", "招商佣金", "活动 + 奖项", "股权分红", "首年合计"],
        [
            ["保守", "360 万", "300 万", "240 万", "60 万", "≈ 960 万元"],
            ["基础", "480 万", "600 万", "390 万", "120 万", "≈ 1,590 万元"],
            ["乐观", "600 万", "1,200 万", "560 万", "240 万", "≈ 2,600 万元"],
        ])
    add_body(s, [
        "三档测算与 02_测算模型/合作收益测算模型.xlsx 完全一致；可作为月费区间谈判锚点。",
    ], top=4.6, height=1.4, font_size=14)
    add_footer(s)
