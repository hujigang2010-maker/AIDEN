...
def main():
    ...
    # ── 6 收费项目总表 ──
    s, p = new_slide()
    slide_header(s, "二、明确收费项目（学生端价目）", "已取消开证明低价档；只做真实企业课题的高价值业务", p, TOTAL)
    add_rounded(s, Inches(0.4), Inches(1.4), Inches(12.5), Inches(0.7), fill=LIGHT)
    add_text(s, Inches(0.6), Inches(1.5), Inches(12.1), Inches(0.5),
             "战略取舍：199 / 699 / 1,680「开证明」业务费用低、核验与舆情成本高 → 一律停售；证明仅作为高价值项目结业交付物，不单独售卖。",
             size=13, bold=True, color=ORANGE, anchor=MSO_ANCHOR.MIDDLE)
    headers = ["收费项目", "标准价(元/人)", "包含", "不包含", "适用人群"]
    rows = [
        ["科技/具身智能项目实习", "5,480", "真实企业课题 4–6 周 + 周报 + 结业证明", "包录取/包转正", "高中生优生/大学生"],
        ["上海 AI / 头部通道营", "11,800", "品牌项目 + 导师 + 答辩 + 结业证明", "包转正/包录取", "大学生优生"],
        ["毕业生就业转化包", "9,800", "过渡实习 + 内推辅导 + 复盘 + 证明", "包 Offer", "毕业生"],
        ["增值：推荐信加购（可选）", "+1,500", "仅限已参加上述项目学员", "不可单独购买", "有申请/求职需要者"],
        ["机构团购", "标准价 6–8 折", "批量名额 + 统一课题交付", "个性化造假包装", "学校/教培 B 端"],
    ]
    make_table(s, Inches(0.35), Inches(2.3), Inches(12.6), Inches(3.8),
               headers, rows, font_size=11)
    add_text(s, Inches(0.5), Inches(6.3), Inches(12.3), Inches(0.4),
             "支付节奏：定金 30% → 开营/企业确认接收 40% → 结业交付 30%。",
             size=13, color=DARK)

    # ── 7 推送成本 ──
    s, p = new_slide()
    slide_header(s, "二、推送至企业的成本（我们花多少）", "「收了学生再推过去」—— 只测算在售高价值档", p, TOTAL)
    ...
    make_table(s, Inches(0.3), Inches(1.5), Inches(12.7), Inches(2.8),
               headers, rows, font_size=12)
    add_bullet_list(s, Inches(0.5), Inches(4.6), Inches(12.3), Inches(2.2),
                    [
                        "企业通道成本 = 与企业/课题方按人头或批次结算的费用（或内部导师课题等价成本）",
                        "首批上海具身智能企业：建议按 2,000–3,200 元/人谈价；AI 头部通道按 3,000–4,200 元/人预留",
                        "不再做低价开证明：避免「费用低、风险高、占用带宽」的业务拖累主航道",
    ...
    rows = [
        ["01–02 报名评估", "招生顾问 + 教务", "报名后 2 个工作日", "高价值档匹配方案确认书"],
        ["03–04 匹配推送", "企业对接人", "方案确认后 5 个工作日", "企业接收确认函"],
        ["05 实习执行", "学员 + 企业导师 + 教务", "按 4–6 周计划", "周报齐全 + 真实课题产出"],
        ["06–07 结业交付", "教务 + 合规", "结业后 3 个工作日", "项目证明（非单独售卖）"],
    ]
    ...
    make_card(s, Inches(4.65), Inches(1.45), Inches(4.0), Inches(5.2),
              "收费与推送成本", [
                  "在售：5,480 / 11,800 / 9,800",
                  "已停：199 / 699 / 1,680 开证明",
