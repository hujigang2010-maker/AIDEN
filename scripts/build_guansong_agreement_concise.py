...
def build():
    ...
    heading(doc, "2026 人工智能商业化落地与硬核投资破局峰会",
            level=2, color=GREY, center=True)
    heading(doc, "《官方智慧出行伙伴合作协议》",
            level=1, color=HUAWEI_RED, center=True)
    blank(doc)
    ...
    clause(doc, "1.3",
           "支付与对价交付方式：现金部分应于本协议签署并经双方盖章生效后 15 个工作日内一次性汇入甲方指定的对公账户（账户信息详见本条第 1.5 款）；实物调用对价部分（车队及司机服务）由乙方在峰会执行期间按本协议第二条约定直接交付。甲方在现金部分到账后向乙方开具相应金额的合规发票（发票内容：会议服务费 / 会务咨询费 / 赞助费 三选一，以乙方实际需求为准）。")
    clause(doc, "1.4",
    ...
    for i, (k, v) in enumerate(acct_rows):
        ...
        shade_cell(acct.rows[i].cells[0], "FBE5E5")
    ...
    clause(doc, "2.4",
           "长效生态延伸权益（战略附赠，不计入第 1.2 条赞助标的额）：（1）乙方自动成为【人工智能商业化落地峰会组委会】举办的后续【见微知海新质商业生态】年度战略理事单位，享有未来半年内组委会举办的其他系列闭门私局的优先参与权；（2）针对后续组委会生态内成功跑通的重资产企业、科创园区及超级个体，如有高端商务用车集中采购或高管换车需求，甲方将优先向乙方及华为鸿蒙智行体系进行线索导入与商业撮合；（3）甲方智库团队在进行「智慧城市与新质人居」相关学术及商业课题调研时，优先将乙方及华为生态作为研究样板与联合发布方。")
    blank(doc)
