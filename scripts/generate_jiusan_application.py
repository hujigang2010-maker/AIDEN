...
def build_basic(doc):
    ...
    value_cell(addr.cell(1, 5), "", align=WD_ALIGN_PARAGRAPH.CENTER)
    ...
    rows = [
        ("上海市杨浦区科技企业联合会", "区级", "执行会长", "上海", "上海", "兼任"),
        ("上海市工商联房地产商会", "市级", "秘书长", "上海", "上海", "兼任"),
        ("复旦管院不动产资产管理协会", "校级/协会", "创始理事长", "上海", "上海", "亦称秘书长"),
        ("上海山东省商会", "市级", "理事", "上海", "上海", ""),
    ]
    ...
    add_para(doc, "上海市人才引进认证；项目管理工程师任职资格（上海市住房和城乡建设委员会评定）。", size=10)
...
def build_family_awards(doc):
    ...
    # 简历未列明确省部级奖项，按填写说明可留空；人才引进写入备注性一行供参考
    values = ["上海市人才引进认证", "/", "/", "其他", "上海市相关主管部门", "非省部级科技奖，供参考"]
    for c, v in enumerate(values):
        value_cell(award.cell(1, c), v, size=8, align=WD_ALIGN_PARAGRAPH.CENTER)
    for c in range(6):
        value_cell(award.cell(2, c), "")
    add_note(doc, "按填写说明，获奖情况原则上填写省部级以上奖励；其余荣誉可另附材料。", size=9)
...
def build_patents_and_achievements(doc):
    ...
    achievements = [
        ...
        "《基于BIM的装配式钢结构建筑施工新技术与管理研究》（《中国……》相关期刊，题名以正式发表稿为准）。",
        "2. 实用新型专利2项：建筑工程管理用功能脚架；建筑工程用防护围栏。",
        "3. 长期从事不动产投资拓展与住房政策研究工作，具备商业综合体、产城融合、工业地产、物流地产、"
        "文旅康养、长租公寓及农业综合体等多业态土地获取与落地经验；熟悉重资产勾地、轻资产品牌输出、"
        "委托代建、股权收并购、法院拍卖与不良资产处置等拓展路径。",
        "4. 代表性项目（节选）：万科靖江印象城（商业综合体+轻资产委管）；嘉兴海宁商住项目；"
        "镇江大港万科金域蓝湾（收并购+债务重组）；常州金坛理想城；临沂河东鲁商新都会；"
        "上海龙湖滟澜山叠拼别墅（法院拍卖收并购）；万纬嘉兴平湖冷链产业园；万纬上海南桥冷链物流园；"
        "新城临沂吾悦广场、青岛吾悦广场；中南如皋世纪城、苏州中南锦苑、江阴白鹭湾等。",
        "5. 现任复旦大学住房政策研究中心秘书长，牵头住房保障、租赁市场、城市更新等课题研究与产学研交流；"
        "兼任杨浦区科技企业联合会执行会长、上海市工商联房地产商会秘书长等，服务科创企业与不动产行业协同发展。",
    ]
