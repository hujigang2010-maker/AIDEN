# -*- coding: utf-8 -*-
"""将《2026 谷歌上海开发者大会白皮书》Markdown 文稿转换为 Word（docx）。"""
import os
import re
...
OUTPUT = os.path.join(BASE, "2026谷歌上海开发者大会白皮书.docx")
LOGO = os.path.join(BASE, "assets/logo_fudan_hprc.png")
...
set_font(fp.add_run("复旦大学住房政策研究中心  ·  FDU-HPRC-WP-2026-03  ·  2026 谷歌上海开发者大会白皮书"),
         cn="宋体", size=8.5, color=GRAY)

# ---------------- 封面 ----------------
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
p.paragraph_format.space_before = Pt(24)
p.add_run().add_picture(LOGO, width=Cm(13.5))

p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
p.paragraph_format.space_before = Pt(48)
set_font(p.add_run("2026 谷歌上海开发者大会"), cn="黑体", en="Arial", size=26,
         bold=True, color=BLUE)
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
set_font(p.add_run("白皮书"), cn="黑体", en="Arial", size=32, bold=True, color=BLUE)

p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
p.paragraph_format.space_before = Pt(22)
set_font(p.add_run("—— 智能体时代的中国开发者生态、出海接口与城市空间含义 ——"),
...
while i < len(lines):
    ...
    if re.match(r"^\*\*(编制单位|文稿系列|文稿编号|成稿日期|副标题)\*\*", line):
        i += 1
        continue
    ...
    if m:
        ...
        if os.path.exists(path):
            ...
            set_font(cap.add_run(m.group(1)), cn="楷体", size=9.5, color=GRAY)
        i += 1
        continue
