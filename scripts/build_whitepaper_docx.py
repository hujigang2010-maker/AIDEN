# -*- coding: utf-8 -*-
"""将《2026 Google 开发者大会观察白皮书》Markdown 文稿转换为 Word（docx）。"""
import os
import re
...
MD = os.path.join(BASE, "2026Google开发者大会观察白皮书.md")
OUTPUT = os.path.join(BASE, "2026Google开发者大会观察白皮书.docx")
...
set_font(p.add_run("2026 Google 开发者大会"), cn="黑体", en="Arial", size=28,
         bold=True, color=BLUE)
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
set_font(p.add_run("观察白皮书"), cn="黑体", en="Arial", size=30, bold=True,
         color=BLUE)

p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
p.paragraph_format.space_before = Pt(22)
set_font(p.add_run("—— 从灵感火花到产品跨海出圈 ——"), cn="楷体", size=14,
         color=GRAY)
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
set_font(p.add_run("智能体时代的全球开发者网络与城市空间含义"), cn="楷体",
         size=13, color=GRAY)
...
while i < len(lines):
    ...
    if line.startswith("**从灵感火花到产品跨海出圈"):
        i += 1
        continue
    ...
    if line.startswith("# "):
        ...
        if not first_h1_skipped:
            ...
            i += 1
            # skip subtitle line immediately following H1 if it is bold-only
            continue
        if chapter_no > 0:
            doc.add_page_break()
        chapter_no += 1
