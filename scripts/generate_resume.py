#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""生成胡继刚优化版简历（Word + 可打印 HTML / PDF）。

基于《胡继刚-简历-优化版0921.pdf》按 2026-09-21 修改建议改写：
联系方式、学历口径、现职课题、北欧会客厅、媒体点名，以及万科 / 新城 / 中南履历。
...
def build_docx() -> Path:
    ...
    add_p(
        doc,
        "土木本科、复旦大学硕士研究生，15 年不动产投资拓展与政府资源整合经验，历任中南、新城、万科投资岗至区域投资副总经理，"
        "熟悉勾地、轻资产输出、代建、收并购与不良资产「募投管退」。2021 年起任职高校智库秘书长，并以杨浦科企联执行会长身份搭建政产学研平台。"
        "近两年公开主持/主办人工智能商业化峰会、中国投资者海外不动产主题活动，并共同发起北欧创新国际会客厅，"
        "形成「AI 产业化落地 + 企业出海对接 + 政府招商落地」复合能力。"
        "公开活动被人民网、新华社、中新社、央广网、上海市人民政府点名。",
        size=9.5,
        color=GRAY,
        space_after=2,
        line=1.18,
    ...
    pillars = [
        ...
        (
            ...
            [
                ...
                "2026.03 共同发起「北欧创新国际会客厅」，落户杨浦宝龙旭辉广场环创中心（宝龙商办不是雇主）。",
    ...
    add_p(
        doc,
        "兼杨浦区科技企业联合会执行会长、复旦大学不动产资产管理协会创始理事长。"
        "牵头城市更新、存量资产与新质产业空间：关注产业如何用空间、政策、资本落地城市。产业方向，不是技术研发岗。",
        size=9,
        color=MUTED,
        space_before=2,
        space_after=2,
    ...
    for t in [
        "平台：2026.03 北欧创新国际会客厅共同发起，落户杨浦宝龙旭辉广场环创中心。宝龙商办不是雇主。公开活动被人民网、新华社、中新社、央广网、上海市人民政府点名。",
        "AI：2026.05 主办方代表，人工智能商业化落地与硬核投资破局峰会（上海北外滩，600+ 嘉宾）；主持「从算力引擎到新质资产（AI 全产业链商业化实战）」圆桌。",
        "出海 / 招商：2024.06 主持中国投资者海外不动产战略布局主题活动；2025.05 联合主办全球新经济增长引擎峰会并参加房地产×资本×产业圆桌。",
    ]:
        bullet(doc, t)
    ...
    add_p(
        doc,
        "上海 · 汇报投资合作开发部总经理 · 团队约 8 人。深耕江浙未进入城市，负责策略勾地、收并购、债务重组与不良资产处置。",
        size=9,
        color=MUTED,
        space_before=2,
        space_after=2,
    ...
    add_p(doc, "上海。负责山东、江苏等区域战略布局与土地获取。", size=9, color=MUTED, space_before=2, space_after=2)
    for t in [
    ...
    add_p(
        doc,
        "上海 / 南通 · 2011.06–2012.10 总裁办战略企划；其后新项目发展中心。",
        size=9,
        color=MUTED,
        space_before=2,
        space_after=2,
    ...
    proj_rows = [
        ("项目", "模式", "要点"),
        ("镇江大港金域蓝湾", "收并购 + 国企合作", "约 199 亩 / 32 万方，与镇江大港集团各半合作"),
        ("嘉兴海宁商住", "政府代建落地", "18.53 亿元 / 约 23 万㎡，配建安置房"),
        ("上海龙湖滟澜山", "法拍不良资产", "测算 IRR 约 16.16%，特资「募投管退」样本"),
        ("万科靖江印象城", "勾地 + 印力委管", "约 15 亿元 / 556 亩 / 购物中心约 7 万方，苏中县市首家印象城"),
        ("常州金坛理想城", "收并购 + 债务重组", "约 28.05 亿元、561 亩，底价获取"),
        ("万纬平湖 / 南桥冷链", "产业勾地 / 收购改造", "嘉兴首个高标准自动化冷链园；上海奉贤南桥智能化园区"),
        ("临沂新城吾悦广场", "市级重点招商", "对接市重点招商政策，集团首进山东的重资产项目"),
        ("青岛新城吾悦广场", "轻资产品牌输出", "品牌输出 + 委管，约 13.68 万㎡"),
    ]
    ...
    blocks = [
        (
            "教育与培训",
            "2018.09–2021.06  复旦大学硕士研究生（工商管理 / 财务金融）\n"
    ...
    add_p(
        doc,
        "说明：公开活动职务与角色可对照人民网、新华社、中新社、央广网、上海市人民政府、杨浦区政府官网、锦天城官网等报道核验。"
        "北欧创新国际会客厅落户杨浦宝龙旭辉广场环创中心，宝龙商办不是雇主。"
        "原 Boss 直聘稿中的身份证、家庭住址、配偶信息已从对外简历中移除。",
        size=8,
        color=MUTED,
        space_before=8,
        space_after=0,
...
def export_pdf(html_path: Path, pdf_path: Path) -> None:
    ...
    dated = OUT / "胡继刚-简历-优化版0921.pdf"
    if pdf_path.name == "胡继刚-简历-优化版.pdf":
        shutil.copy2(pdf_path, dated)
