...
def build(path: Path | None = None) -> Path:
    ...
    body(
        doc,
        "甲乙双方拟在上海就企业出海主题活动、以及高端教育招生宣传进行合作。为明确各自工作、费用和交付事项，订立本协议。本协议签署前供乙方审阅，签署后生效。",
    )

    h(doc, "第一条　合作事项")
    body(
        doc,
        "1.1 双方合作举办一场以企业出海为主题的专题课，地点在乙方上海中心（外滩SOHO F栋），人数三十至四十人。来宾主要为正在考虑出海或赴港上市的企业负责人。乙方安排场地和老师，甲方负责邀请和现场。课后由乙方招生人员与来宾单独沟通。录取、面试、学位由乙方按学院规定办理，甲方不得代为承诺。",
    )
    body(
        doc,
        "1.2 甲方另安排一场出海主题活动，请乙方联系人转达，邀请乙方上海中心主任出席。主任是否出席，以乙方安排为准。甲方发出书面邀请，活动如期举办，即完成本项工作。",
    )
    body(
        doc,
        "1.3 港大本部出海课程的推广、原创谷、学生在沪实习参访等事项，双方可另行商议，不列入本协议交付，也不另收费。",
    )
    body(
        doc,
        "1.4 本协议不是招生代理，不按学费或录取人数分成，也不是联合办学。任何一方不得称对方为自己的下属机构，也不得对外宣称双方合办学位。",
    )

    h(doc, "第二条　各自工作")
    body(doc, "2.1 乙方负责：提供上海中心场地和名义；安排老师或校友；招生跟进；协助邀请中心主任；决定录取和学位。")
    body(doc, "2.2 甲方负责：九十天内的筹备；拟邀请名单；外滩专题课现场；请主任出席的那一场活动；课后七日内提交纪要和名单；开具发票、收取费用。")
    body(
        doc,
    ...
    body(
        doc,
        "4.1 甲方提交拟邀请名单初稿，人数不少于八十人。乙方在五个工作日内提出书面意见，逾期视为没有意见。名单供工作使用，不保证每一个人都到场。",
    )
    body(
        doc,
        "4.2 专题课完成后七日内，甲方提交纪要和来宾情况说明。因不可抗力、乙方改期或乙方未按时确定老师，影响人数的，不视为甲方违约。",
    )
    body(
        doc,
        "4.3 请主任出席一项，以甲方书面邀请为凭。主任未出席，不视为甲方违约。",
    )
    body(
        doc,
