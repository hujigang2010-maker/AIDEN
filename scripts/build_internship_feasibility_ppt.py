...
def main():
    ...
    add_text(s, Inches(0.9), Inches(5.6), Inches(11.5), Inches(0.35),
             "商业实习项目 × 公益补充服务  |  定价机制与盈利测算  |  PPT + Excel 双交付",
             size=13, color=WHITE)
    ...
    rows = [
        ["高中生", "海外/名校申请加分", "实习证明、推荐信、项目报告", "中高（家长决策）", "大学录取 / 竞赛履历"],
        ["大学生", "丰富履历、校招竞争力", "大厂/科技公司实习经历", "中（个人+家庭）", "暑期实习转正 / 校招"],
    ...
    add_text(s, Inches(0.5), Inches(5.1), Inches(12.3), Inches(1.5),
             [
                 "共性需求：可验证的实习经历 + 可出具的证明材料 + 可写入简历/文书的项目成果",
                 "差异策略：高中生偏「证明与推荐」；大学生偏「品牌与技能」；毕业生偏「就业转化通道」",
             ],
             size=14, color=DARK)
    ...
    # ── 8 产品总览 ──
    s, p = new_slide()
    slide_header(s, "三、产品与项目规划总览", "商业主业 + 公益补充，形成漏斗", p, TOTAL)
    make_card(s, Inches(0.5), Inches(1.5), Inches(6.0), Inches(5.2),
              "A. 商业实习项目（利润中心）", [
                  "世界 500 强 / 知名大厂合作通道",
                  "机器人与科技公司实岗/课题",
    ...
    # ── 11 公益项目 ──
    s, p = new_slide()
    slide_header(s, "五、公益类实习与增值支持", "补充点：低门槛获客 + 证明服务闭环", p, TOTAL)
    make_card(s, Inches(0.5), Inches(1.5), Inches(4.0), Inches(5.2),
              "(a) 线上实习盖证明", [
                  "统一线上任务平台",
                  "每周打卡 + 成果提交",
    ...
    # ── 12 定价 ──
    s, p = new_slide()
    slide_header(s, "六、收费标准与定价机制", "分层套餐，覆盖三类人群支付能力", p, TOTAL)
    headers = ["产品线", "套餐", "建议定价（元/人）", "包含内容", "目标人群"]
    rows = [
        ["公益基础", "线上实习 · 证明版", "0–299", "任务+证明编号", "高中生/预算敏感"],
        ["公益增值", "证明 + 申报支持", "499–899", "邮箱通道+材料清单", "高中申请党"],
        ["公益精品", "证明 + 推荐信协助", "1,280–1,980", "模板+润色+导师签字流", "海外申请"],
        ["商业标准", "科技公司项目实习", "3,980–6,980", "4–6周项目+证明+周报", "大学生"],
        ["商业进阶", "大厂/500强通道营", "8,800–15,800", "品牌项目+导师+答辩", "大学生/优生"],
        ["就业转化", "毕业生过渡实习包", "6,800–12,800", "实习+内推辅导+复盘", "毕业生"],
        ["机构合作", "学校/教培团购", "按人 6–8 折", "批量名额+定制证明", "B 端渠道"],
    ]
    make_table(s, Inches(0.35), Inches(1.45), Inches(12.6), Inches(5.2),
               headers, rows, font_size=11)
    ...
    make_card(s, Inches(0.5), Inches(1.5), Inches(6.0), Inches(2.4),
              "价格锚点", [
                  "以「证明可信度 × 品牌背书 × 辅导强度」三维定价",
                  "公益线做漏斗入口，商业线贡献 ≥ 75% 毛利",
    ...
    # ── 14 盈利测算 ──
    s, p = new_slide()
    slide_header(s, "七、盈利模型与财务测算（试点年）", "保守 / 基准 / 乐观三情景，详见 Excel", p, TOTAL)
    headers = ["情景", "年付费人数", "客单价（均）", "营收（万元）", "综合毛利率", "毛利（万元）"]
    rows = [
        ["保守", "120", "4,500", "54", "32%", "17.3"],
        ["基准", "220", "5,800", "127.6", "38%", "48.5"],
        ["乐观", "360", "6,500", "234", "42%", "98.3"],
    ]
    ...
    add_bullet_list(s, Inches(0.6), Inches(4.4), Inches(12), Inches(2.4),
                    [
                        "结构假设（基准）：公益线 40% 人数 / 约 12% 收入；商业线 50% 人数 / 约 70% 收入；就业包 10% 人数 / 约 18% 收入",
                        "盈亏平衡粗算：固定成本约 28–35 万/年（运营 2–3 人 + 平台 + 合规），基准情景可覆盖并实现盈余",
                        "关键敏感因子：大厂通道成本、获客 CAC、公益→商业转化率；Excel 提供可调参数表",
                    ],
                    size=14, color=DARK)
    ...
    rows = [
        ["M1", "基建", "产品封装、证明模板、合规话术、官网/表单", "MVP 上线"],
        ["M2", "供给", "签约科技公司 5–8 家，确认导师池", "可售库存就绪"],
        ["M3", "获客", "留学机构/高校社团渠道试点，公益线放量", "首批 40 人开营"],
        ["M4", "放量", "大厂通道营首期，收集案例与口碑", "付费累计 100 人"],
    ...
    # ── 16 风险与结论 ──
    s, p = new_slide()
    slide_header(s, "九–十、风险合规与论证结论", "先守住可信，再谈规模", p, TOTAL)
    ...
    make_card(s, Inches(6.8), Inches(1.45), Inches(5.9), Inches(3.3),
              "可行性结论（建议）", [
                  "需求真实：三类人群痛点清晰且付费场景成立",
                  "供给可建：科技公司+线上公益可快速起盘",
                  "盈利路径清晰：商业线扛利润，公益线扛规模",
                  "建议：按基准情景启动 6 个月试点验证",
                  "配套：使用 Excel 表做报价、清单与测算跟踪",
