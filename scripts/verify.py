...
def main() -> None:
    for key, path in FILES.items():
        min_size = 200 if key == "wx" else 1000
    ...
    if len(prs.slides) != 12:
        fail(f"PPT 页数应为 12，实际 {len(prs.slides)}")
    ok("PPT 12 页")
    ppt_text = text_of_pptx(FILES["ppt"])
    for needle in (C.PROJECT_NAME, C.THEIR_CONTACT, "88,000", "前期", "潘嘉琰", "外滩"):
        if needle not in ppt_text:
            fail(f"PPT 缺少关键词：{needle}")
    ok("PPT 含思路、接口人与前期费用")

    agr = text_of_docx(FILES["agreement"])
    for needle in (C.FEE_AMOUNT_CN, "一次性", "不是招生代理分成", C.THEIR_EMAIL, "第四条"):
    ...
    need = ["00-封面", "01-前期费用构成", "02-90天交付清单", "03-首场执行", "04-双方分工", "05-付款与开票"]
    if wb.sheetnames != need:
        fail(f"Excel 工作表不符：{wb.sheetnames}")
    ws = wb["01-前期费用构成"]
    if ws["C9"].value != "=SUM(C5:C8)":
        fail("费用合计公式不正确")
    ok("Excel 六张表且合计公式正确")
    ...
    if "8.8" not in wx and "88,000" not in wx and "捌万捌仟" not in wx:
        fail("沟通稿未写费用")
    if C.THEIR_CONTACT not in wx:
        fail("沟通稿未写对方姓名")
    if "前期" not in md:
        fail("Markdown 未写前期费用")
    ok("Markdown 与沟通稿完整")
    print("ALL PASSED")
