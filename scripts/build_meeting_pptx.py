...
def build():
    ...
    add_chrome(s, prs, page_no=4, label="收费",
               title="还是这三笔，柔和一点",
               subtitle="数字不改方向。启动费抵佣金、首月含在启动费里、有成交抵月度、链主不上浮。")

    fees = [
        ("启动费", "15 万", "一次性进场\n含第一个月 · 抵扣佣金"),
        ("月度费", "8 万 / 月", "第 2 个月起\n当月有佣金则抵扣"),
        ("成功佣金", "首年租金 8%", "扣税、物业、免租\n链主也是 8% · 没签成不抽"),
    ]
    ...
    add_text(s, Inches(0.7), Inches(4.75), Inches(12.0), Inches(1.7),
             "中介带来的单，我们只按 8% 的一半（4%）计，不跟中介各收满额。\n"
    ...
    add_chrome(s, prs, page_no=5, label="签法",
               title="建议先勾 90 天",
               subtitle="收费仍是 15 万 + 8 万/月 + 8%。先看人干活，再锁 12 个月。")
