...
def build_leasing_mandate():
    ...
    add_p(
        doc,
        "7.2 核心组：乙方应维持不少于 3 人的招商操盘核心组（项目经理 + 招商 + 材料/商务）。"
        "人员名单于进场 5 个工作日内书面提交。",
        indent=True,
    )
    ...
    add_h1(doc, "第九条 · 服务费用（三笔账，缺一不可）")
    add_p(
        doc,
        "9.1 费用结构对应『进场 / 干活 / 结果』，避免只开会、不招商。下列金额为建议口径，"
        "签署前填入附件 E 确认单，以附件 E 为准。",
        indent=True,
    )
    ...
    add_p(
        doc,
        "9.2 启动费人民币 [壹拾伍万元整]（建议区间 10–20 万元）。"
        "本合同签署后 10 个工作日内一次性支付。用于覆盖进场前已完成的业态方案、"
        "案例研究、招商材料及进场筹备。启动费可全额抵扣后续应付成功佣金，不退。",
        indent=True,
    )

    add_h2(doc, "（二）月度操盘费 · 干活")
    add_p(
        doc,
        "9.3 月度操盘费人民币 [捌万元整]/月（建议区间 6–12 万元/月），覆盖第 7.2 条核心组。"
        "每月 5 日前支付当月费用，从进场月起算，满月计付；不足月按日折算。",
        indent=True,
    )
    add_p(doc, "9.4 月度操盘费不含下列甲方另行承担的成本：", indent=True)
    add_bullet(doc, "第三方中介居间佣金")
    add_bullet(doc, "发布会/活动场地、餐饮、搭建、媒体硬广")
    add_bullet(doc, "样板间装修及工程改造")
    add_bullet(doc, "政府关系中需由甲方主体出具的文件、保证金、税费")

    add_h2(doc, "（三）成功佣金 · 招到才收")
    add_p(
        doc,
    ...
    add_p(doc, "9.6 费率：", indent=True)
    add_bullet(doc, "常规租户（乙/丙档）：基数 × 8%")
    add_bullet(doc, "链主/锚定租户（甲档）：基数 × 12%")
    add_bullet(doc, "冠名或对赌租户（丁档）：基数 × 10%，另加冠名年费（如有）的 5%")
    add_p(
        doc,
        "9.7 支付时点：租赁合同生效且甲方收到履约保证金或首期租金后 15 个工作日内，"
        "甲方支付对应成功佣金。启动费抵扣在第一笔成功佣金中优先扣减。",
        indent=True,
    )
    add_p(
        doc,
        "9.8 退租扣回：承租人于起租日后 12 个月内因可归责于招商质量的原因（如乙方明知"
        "客户不具备履约能力仍推荐、或用途虚假）导致合同解除的，乙方按未履行租期占首年"
        "比例退还已收成功佣金。因甲方违约、房屋瑕疵、政策变化、客户自身战略收缩导致的"
        "退租，不扣回。",
        indent=True,
    )
    add_p(
        doc,
        "9.9 中介成交：经乙方报备系统登记的中介成交，甲方另付中介佣金；"
        "同时向乙方支付第 9.6 条成功佣金的 50%（乙方履行了统管、冲突裁决与谈判协同）。"
        "未经报备的中介成交，乙方不享有该笔成功佣金。",
        indent=True,
    )
    add_p(
        doc,
        "9.10 税费：本条金额均为未税价，增值税及附加按法定由甲方在支付时另付，"
        "乙方开具合法发票。",
        indent=True,
    )
    add_p(
        doc,
        "9.11 M12 未达第 8.1 条结果指标但双方续约的：第 13–18 个月度操盘费下浮 20%，"
        "成功佣金费率不变。",
        indent=True,
    )
    ...
    add_kv_table(doc, [
        ("进场日", "[YYYY-MM-DD]"),
        ("启动费（未税）", "人民币 [150,000] 元"),
        ("月度操盘费（未税）", "人民币 [80,000] 元/月"),
        ("常规成功佣金", "首年租金基数 × [8]%"),
        ("链主/锚定成功佣金", "首年租金基数 × [12]%"),
        ("丁档成功佣金", "基数 × [10]% + 冠名年费 × [5]%"),
        ("启动费是否抵扣成功佣金", "是 / 否（默认是）"),
