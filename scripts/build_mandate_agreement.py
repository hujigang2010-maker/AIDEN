...
def build_leasing_mandate():
    ...
    add_p(
        doc,
        "7.2 核心组：试跑 90 天不少于 2 人（项目经理 + 招商）。转正后维持不少于 3 人。"
        "人员名单于进场 5 个工作日内书面提交。",
        indent=True,
    )
    ...
    add_h1(doc, "第九条 · 服务费用（对齐后：试跑包干 + 佣金跟中介同一尺）")
    add_p(
        doc,
        "9.1 上一版『启动费 + 高月度 + 首年租金百分比』叠在一起偏高。"
        "现改为：试跑 90 天只付包干；成交按首月不含税净租金计佣；"
        "中介单业主只付一套。金额为建议口径，以附件 E 为准。",
        indent=True,
    )

    add_h2(doc, "（一）90 天包干 · 试跑期不另收月度")
    add_p(
        doc,
        "9.2 试跑期包干人民币 [捌万元整]。本合同签署后 10 个工作日内一次性支付。"
        "覆盖试跑期内人员、材料、带看。试跑期不另收月度操盘费。"
        "包干可全额抵扣后续应付成功佣金，不退。",
        indent=True,
    )

    add_h2(doc, "（二）转正后月度操盘费")
    add_p(
        doc,
        "9.3 双方书面确认转正（或选择方案 A 自第 91 日起）后，月度操盘费人民币 [肆万元整]/月。"
        "每月 5 日前支付当月。当月若产生应付成功佣金，月度费从该笔佣金中抵扣。",
        indent=True,
    )
    add_p(doc, "9.4 月度与包干均不含：", indent=True)
    add_bullet(doc, "第三方中介居间佣金（已含在第九条『一套佣金』内）")
    ...
    add_p(
        doc,
        "9.5 计算基数 = 生效租赁合同项下成交首月不含税净租金"
        "（面积 × 合同日租金 × 30，不含增值税、不含物业费）。免租期不减少该分子。",
        indent=True,
    )
    add_p(doc, "9.6 费率：", indent=True)
    add_bullet(doc, "乙方自拓常规租户：基数 × 100%")
    add_bullet(doc, "链主/锚定（单户 ≥ 1,500 ㎡ 或整层或书面确认甲档）：基数 × 150%")
    add_p(
        doc,
    ...
    add_p(
        doc,
        "9.9 中介成交：甲方就该笔向中介与乙方支付的成功佣金合计，不超过第 9.6 条该档。"
        "内部分成原则为中介约 70%、乙方约 30%，以报备系统登记为准。"
        "未经报备的中介成交，乙方不享有分成。",
        indent=True,
    )
    add_p(
    ...
    add_p(
        doc,
        "9.11 试跑期满未转正的：之后不产生月度费；已发生成功佣金照付；包干不退。",
        indent=True,
    )
    ...
    add_kv_table(doc, [
        ...
        ("90 天包干（未税）", "人民币 [80,000] 元，试跑期不另收月度"),
        ("转正后月度操盘费（未税）", "人民币 [40,000] 元/月，当月成功佣金可抵扣"),
        ("自拓成功佣金", "首月不含税净租金 × [100]%"),
        ("锚定/链主成功佣金", "首月不含税净租金 × [150]%"),
