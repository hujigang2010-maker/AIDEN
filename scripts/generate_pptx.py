...
def build_ppt(output_path: Path) -> None:
    ...
    # 8 医院
    s = prs.slides.add_slide(blank)
    header(s, "治疗不要断档", "手术已经在三甲做完，这是有利起点")
    add_bullets(
        s,
        Inches(0.55),
        Inches(1.3),
        ...
        Inches(5.5),
        [
            "8月14日急诊骨科 CT → 8月15日手足与显微重建外科术后 CT：齐鲁医院三甲闭环。",
            "交通事故就医默认三甲。转二甲（市北区人民医院）有理赔不确定风险。",
            "交警说同路段有二甲先例，但不能书面保证。要留痕，不能只听「应该没问题」。",
            "跨院必须先结算再入院。这是硬规则。",
            "赛主任要交警电话：可以给，公对公。不要家属扮演交警。",
            "目标：尽量留在或转回三甲连续治疗；若暂留二甲，书面确认接收交通事故伤者。",
        ],
        size=16,
        spacing=1.22,
    )
    ...
    # 9 本周
    s = prs.slides.add_slide(blank)
    header(s, "本周只做六件事", "残级、总包死价，本周不谈")
    items = [
        ("1", "存 CT", "三份报告、检查号、影像入口全部下载"),
        ("2", "要病历", "入院、手术、医嘱、费用明细、发票"),
        ("3", "不断治", "按医院规则结算，避免无法入院"),
        ("4", "医院级别", "交警与赛主任公对公留痕"),
