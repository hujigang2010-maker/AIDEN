...
def build_ppt(output_path: Path) -> None:
    ...
    # 8 医院
    s = prs.slides.add_slide(blank)
    header(s, "转院报销卡住了", "人已到人民医院，齐鲁出院未办，二甲入院办不成")
    add_bullets(
        s,
        Inches(0.55),
        Inches(1.3),
        ...
        Inches(5.5),
        [
            "今天第一优先：办齐鲁出院结算，再办人民医院入院。人在床上没有住院号最危险。",
            "医保能报，不等于美团平台险能报。两条分开问。向保险报医院全称。",
            "对方是外卖电动车，没有交强险。认定单上未必有人保/平安。找美团平台险。",
            "术后两天不是康复期。8月15日CT仍写软组织肿胀，不能按朋友说的「已经康复」处理。",
            "院方说联合体、同一家医院；父亲好说话才同意。主治强硬，青岛没有上海那种关系。",
            "不做：装病、出院再入院绕考核、代扮交警、另找别的三甲碰运气。",
        ],
        size=16,
        spacing=1.22,
    )
    ...
    # 9 本周
    s = prs.slides.add_slide(blank)
    header(s, "本周只做六件事", "先打通住院身份。残级、总包死价不谈")
    items = [
        ("1", "办出院入院", "齐鲁结算 → 人民医院入院，避免空窗"),
        ("2", "问美团险", "不是交强险。报医院全称：市北区人民医院"),
        ("3", "存病历发票", "三份 CT、手术记录、费用明细"),
        ("4", "公对公留痕", "交警电话给赛主任，不换号不代扮"),
        ("5", "保险进群", "本周见到骑手，平台和保险进群"),
        ("6", "只谈已发生费用", "不谈残级，不签一次性了结"),
    ]
