...
def build():
    ...
    add_para(doc,
             [("2、如本次活动的赞助方式为提供赞助费的，赞助费总额为¥：", False),
              ...
              ("壹拾万", True),
              ("元整（含税）。", False),
              ("乙方上海市杨浦区科技企业联合会系民办非企业单位（民办非企业法人），按小规模纳税人税务管理，按小规模纳税人简易计税方式适用 1% 增值税征收率向甲方开具增值税普通发票（不开具增值税专用发票）。合同总价款 100,000 元保持不变，其中不含税价款约 99,009.90 元，增值税税金约 990.10 元，乙方实际到账金额相应为含税价款。", True)])
    add_para(doc,
