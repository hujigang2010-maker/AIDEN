...
def build():
    ...
    nr3 = note_p.add_run("处均为根据"
                         ...
                         "合作实际信息补充填入，请乙方法务/财务/业务侧分别 review 后确认。"
    ...
    add_para(doc,
             [("乙方：", False),
              ("人工智能商业化落地峰会组委会 / 上海市杨浦区科技企业联合会", True)],
             line_spacing=2.0)
    add_para(doc,
             [("签约地点：", False),
              ("上海市", True)],
    ...
    add_heading_line(doc, "乙方：人工智能商业化落地峰会组委会 / 上海市杨浦区科技企业联合会（正式签约与盖章主体：上海市杨浦区科技企业联合会）", size=11)
    add_para(doc,
             [("住所：", False),
    ...
    add_para(doc,
             [("5、甲方有权委派", False),
              ("6", True),
              ("名领导/员工代表参与本次活动，并发表讲话，向参会人员介绍和推广甲方及其关联公司、运作的项目、企业文化和成就等。（其中现场服务 2 人 + 销售 2 人 + 甲方指派代表 2 人；晚宴实际出席 2 人；具体名单于活动前 3 个工作日双方确认）", True)])
    ...
    add_para(doc,
             [("1) 甲方拥有本次活动", False),
              ...
              ("晚宴专场", True),
              ("统一名称为\u201C", False),
              ("晚宴冠名战略合作伙伴：________________________________（待甲方填入冠名）", False),
              ("\u201D，对外宣传及媒体报道时", False),
              ("晚宴部分", True),
    ...
    add_para(doc,
             [("4) 其他：", False),
              ("（a）甲方享有主会场 1 处展位、晚宴 KV / 桌卡 / 席卡项目 logo 植入；（b）甲方指派代表享有晚宴前 15 分钟项目专场宣讲（PPT 主讲位置）；（c）甲方享有 500 份手拎袋夹页（项目折页 + 285/310 户型图）；（d）甲方享有论坛后主持人鸣谢口播及现场动线引导，配合甲方意向嘉宾前往项目案场参观；（e）甲方享有朋友圈九宫格项目鸣谢及《赞助权益执行回执》（详见附件）。", True),
              ("。", False)])
    ...
    add_para(doc,
             [("2、", False),
              ("（鉴于第四条第 1 项乙方义务已调整为「不就具体出席率、参会名单及嘉宾构成作硬性数量或人员一致性承诺」，本款关于出席率／嘉宾一致率的违约责任相应不再适用，留为占位以备后续双方另行书面约定。）", True)])
    add_para(doc,
             [("3、", False),
              ("（鉴于第四条第 1 项乙方义务已调整，本款关于「未在签订时提供参会名单」的违约责任相应不再适用，留为占位以备后续双方另行书面约定。）", True)])
    ...
    add_para(doc,
             [("乙方（盖章）：", False),
              ("上海市杨浦区科技企业联合会（正式签约与盖章主体；峰会组委会作为业务合作方共同执行）", True)],
             line_spacing=2.0)
    blank(doc, 4)
    add_para(doc,
             [("法定代表人或授权代表（签名或盖章）：", False),
