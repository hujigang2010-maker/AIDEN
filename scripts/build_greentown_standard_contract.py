...
def build():
    ...
    add_para(doc,
             [("（2）", False),
              ("3% 简易征收开票：", True),
              ("若乙方就本合同项下收入因税务政策调整或资质未办妥等原因不符合免税条件，按小规模纳税人简易计税方式适用 3% 增值税征收率（或当时适用的减按 1% 等小规模纳税人优惠征收率）开具增值税普通发票，合同总价款 100,000 元保持不变，其中不含税价款约 97,087.38 元，增值税税金约 2,912.62 元（按 3% 测算；如按 1% 减征则税金为 990.10 元），乙方实际到账金额相应为不含税价款。", True)])
    add_para(doc,
