...
def build():
    ...
    nr3 = note_p.add_run("处均为根据"
                         ...
                         "其余下划线及空白处保留为待确认项。乙方收款账户与签约主体已根据"
                         "「2026 AI 商业化峰会·腾讯云战略协办协议」已盖章签署版同步确认。")
    ...
    add_heading_line(doc, "乙方：人工智能商业化落地峰会组委会 / 上海市杨浦区科技企业联合会（联合主办；正式签约与盖章主体：上海市杨浦区科技企业联合会）", size=11)
    add_para(doc,
             [("住所：", False),
    ...
    add_para(doc,
             [("（2）本次活动结束并经甲方验收合格后", False),
              ("15", True),
              ("个工作日内一次性支付。", False),
              ("（鉴于活动已于 2026 年 5 月 22 日实际完成，本次合作采用方式 (2) 后置一次性付款）", True)])
    add_para(doc,
             [("（3）其他方式：", False),
    ...
    add_para(doc,
             [("1、", False),
              ("乙方应按本合同约定如期、合规组织本次活动，并配合甲方完成 logo、品牌简介、川总宣讲 PPT、户型折页等权益落地所需的素材确认与上线工作；乙方对参会嘉宾结构与现场组织尽合理之力，但不就具体出席率、参会名单及嘉宾构成作硬性数量或人员一致性承诺。", True)])
