...
def build(path: Path | None = None) -> Path:
    ...
    body(
        doc,
        "鉴于乙方上海中心已启用，需要可持续的产业高管到场机制与符合高级管理人员工商管理学硕士"
        "（EMBA）画像的生源组织；鉴于甲方具备上海科技企业、产业议题策划与闭门活动执行能力。"
        "双方本着平等自愿、优势互补原则，就「外滩·产业课堂」联合策划及前期服务事宜订立本协议。"
    ...
    body(
        doc,
        "1.1 双方联合策划并执行以产业闭门课为载体的招生场景：乙方输出学位品牌、场地与招生流程，"
        "甲方输出议题、定向高管邀约与现场统筹。来宾因产业议题到场，会后由乙方招生官跟进，录取权始终在乙方。",
    )
    body(
        doc,
        "1.2 本协议性质为联合策划服务，不是招生代理分成、不是联合办学、不是学位项目合作。"
        "任何一方不得对外宣称对方为其从属机构，或暗示存在联合学位、互认学分等安排。",
    )
    body(
        doc,
        "1.3 商业结构从简：乙方仅向甲方支付一笔前期联合策划服务费；本协议不约定学费、报名费、"
        "留位费分成，也不按录取人头计酬。",
    )

    h(doc, "第二条　合作内容")
    body(doc, "2.1 合作产品名称为「外滩·产业课堂」，首场举办地点原则上为乙方上海中心（外滩SOHO F栋）。")
    body(
        doc,
        f"2.2 本协议服务周期为自生效日起 {C.PLAN_DAYS} 日。甲方在周期内完成联合策划，并落地首场闭门课（规模 {C.FIRST_EVENT_SIZE}）。",
    )
    ...
    body(
        doc,
        "3.3 乙方指定潘嘉琰女士为日常接口人，变更接口人应提前 3 个工作日书面通知。"
        "甲方指定项目经理一名，于生效后 3 个工作日内书面告知乙方。",
    )
    body(
        doc,
    ...
    body(
        doc,
        "5.2 首场验收：完成场地布置、签到、议程执行，并在结束后 7 日内提交纪要与 A/B/C 分级名单。"
        "到场人数受不可抗力、乙方改期或乙方未按期确认嘉宾影响的，不视为甲方违约。",
    )
    body(
        doc,
        "5.3 验收关注组织与交付，不将录取人数、学费进账作为验收条件。策划方案所列 KPI 为工作目标，"
        "除「提交纪要与分级名单」外，不构成违约条款。",
    )
