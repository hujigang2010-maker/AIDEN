...
def build() -> tuple[Path, Path, Path]:
    ...
    add_heading_cn(doc, "二、三份方案请统一补齐的内容")
    md("## 二、三份方案请统一补齐的内容")
    md()
    ...
    add_paragraph(
        doc,
        "“民间交流、不承担官方外交职能”需要落到合同条款，不能只写在方案开头。请统一对外口径，并补齐下列底线：",
    )
    ...
    add_paragraph(
        ...
        "也正因涉及国资物业、全额装修、补助和公共产业服务，合规要求最高。请补充：",
    )
    md("创智汇最适合做长期产业旗舰，但国资合规要求最高。")
    md()
    add_bullet(doc, "1000㎡专属空间为什么必要，能否先以现有会议室、路演厅按次使用替代。", bold_prefix="面积必要性：")
    add_bullet(doc, "国有物业使用、装修投资、补助或采购所适用的决策、审计和资产管理程序，以及合同主体（北岛科技 / 杨浦科创集团）。", bold_prefix="国资程序：")
    ...
    add_bullet(doc, "建议下一版改为：先借用现有会议空间试点，不立即装修或交付专属场地。", bold_prefix="建议改法：")
    md("- **面积必要性：** 说明 1000㎡为何必要。")
