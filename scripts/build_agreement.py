...
def build():
    ...
    clause(
        doc, "2.4",
        "甲方确认在本次峰会期间不再向同一行业（房地产开发与销售、品质改善型住宅项目）的其他企业授予「晚宴冠名战略合作伙伴」身份及「主背景板钻石级 logo 位」；除此之外其他级别（如黄金、铂金、基础曝光等）的合作不在排他范围。如需引入同行业其他同级以上赞助商，应事先以书面形式告知乙方并取得乙方书面同意。",
    )
    blank(doc)
    ...
    heading(doc, "（三）现场展位权益", level=2)
    add_table(
        doc,
        headers=["序号", "权益内容", "数量 / 规格", "责任方"],
        rows=[
            ["14", "现场专属品牌展位（电梯口）— 含场地及品牌桌台轻包装", "1 处（销售 2 人由乙方派驻；易拉宝、KT 板、户型物料由乙方提供）", "甲方提供场地 / 活动公司基础包装"],
            ["15", "现场专属品牌展位（展场内）— 含场地及洽谈圆桌轻包装", "1 处（销售 2 人由乙方派驻；易拉宝、KT 板、户型物料由乙方提供）", "甲方提供场地 / 活动公司基础包装"],
            ["16", "论坛 / 颁奖结束后主持人鸣谢口播 + 现场动线引导", "1 次主口播 + 现场引导员 1 名", "甲方"],
        ],
        col_widths=[1.2, 8.5, 5.0, 3.0],
    )
    blank(doc)

    heading(doc, "（四）宣发与回执权益", level=2)
    add_table(
        doc,
        headers=["序号", "权益内容", "数量 / 规格", "责任方"],
        rows=[
            ["17", "朋友圈九宫格 — 至少 3 张含项目 logo / KV", "9 张 1080×1080 PNG", "甲方媒体组"],
            ["18", "回顾视频片头 / 片尾项目鸣谢（≤ 3 秒露出，包含项目 logo）", "1 条 1920×1080 mp4", "甲方"],
            ["19", "执行回执：现场图片、嘉宾合影、1 份执行报告", "大会结束后 7 个自然日内出具", "甲方"],
        ],
        col_widths=[1.2, 8.5, 5.0, 3.0],
    )
    blank(doc)

    heading(doc, "（五）乙方自办事项（不计入第 2.2 条赞助金额，由乙方自行承担费用）", level=2)
    add_table(
        doc,
        headers=["序号", "事项", "说明", "责任方"],
        rows=[
            ["自-1", "项目案场接驳：华为尊界 8–10 辆 + 考斯特补位", "往返时长不低于 60 分钟；建议采用「会后专场」（5/22 20:30+）", "乙方"],
            ["自-2", "项目案场接待：售楼处沙盘 + 样板间参观（含夜场灯光秀，如适用）", "由乙方安排并承担费用", "乙方"],
            ["自-3", "现场销售人员", "电梯口 + 展场内合计 4 人，统一品牌服装与胸卡", "乙方"],
            ["自-4", "现场展位升级包装（如背景墙、特装等）", "在甲方提供的基础包装上叠加，由乙方自费", "乙方"],
        ],
        col_widths=[1.2, 4.5, 9.0, 3.3],
    )
    para(
        doc,
        "说明：上述「乙方自办事项」为乙方主动承办的附加投入，与本协议第 2.2 条之赞助金额无关，亦不构成甲方对乙方的支付义务或承诺。",
        size=10, color=GREY,
    )
    blank(doc)
    ...
    # ====== 附表一 ======
    doc.add_page_break()
    heading(doc, "附表一  晚宴冠名战略合作伙伴专项服务报价单 & 执行清单",
            level=0, color=GREEN, center=True)
    para(
        doc,
        "Annex 4 — Schedule 1: Service Quotation & Execution Checklist",
        align="center", size=10, color=GREY, first_indent=False,
    )
    blank(doc)
    para(
        doc,
        "本附表为本协议第 2.2 条「人民币壹拾万元整（￥100,000.00）」赞助费的服务项目价格拆解与执行清单合二为一版本，同时具备「报价单」与「执行清单」双重效力。请甲乙双方在每行右侧「双方确认」列签字 / 盖章；如该项执行后另行变更，应经双方书面确认。",
    )
    blank(doc)

    rows = [
        ["A", "晚宴冠名核心权益（小计：￥50,000）", "", "", "", "", ""],
        ["A1", "晚宴冠名权全程统称 + 晚宴 LED 大屏 KV 主视觉锁屏（入场/餐间/散场全场景）", "1 套（16:9 1920×1080，条屏另出 6144×1080，以场地像素比为准）", "甲方", "15,000", "1", "□ 已确认"],
        ["A2", "晚宴席卡 logo + 项目主张植入", "全场（90×55mm 双面）", "甲方", "4,000", "1", "□ 已确认"],
        ["A3", "晚宴桌卡（桌号牌）logo + 「绿城·潮鸣外滩 · 晚宴冠名」字样植入", "全场（A5 折立 148×210mm 或亚克力 200×150mm）", "活动公司", "4,000", "1", "□ 已确认"],
        ["A4", "晚宴菜单 logo 植入（封面整版 + 内页页脚）", "全场（210×285mm 或 285×210mm）", "活动公司", "3,000", "1", "□ 已确认"],
        ["A5", "晚宴期间项目宣传片轮播（含 LED 上屏与音视频运维）", "≤ 60 秒循环（全晚宴时段）", "甲方上屏 / 乙方供片", "6,000", "1", "□ 已确认"],
        ["A6", "晚宴主持人口播 2 段（开场鸣谢 + 散场鸣谢）", "每段约 30 秒，共 2 段", "甲方", "4,000", "1", "□ 已确认"],
        ["A7", "晚宴前川总专场宣讲位置（含 LED 大屏使用、话筒×2、切换器及现场技术支持）", "15 分钟 + Q&A 5 分钟（18:15–18:30，最终以现场议程为准）", "甲方位置/设备 + 乙方主讲", "6,000", "1", "□ 已确认"],
        ["A8", "晚宴入场券（主桌 3 + 销售/接待 5）", "合计 8 张（最终名单 5 月 19 日前互相确认）", "甲方", "1,000", "8", "□ 已确认"],
        ["B", "主会场植入（小计：￥30,000）", "", "", "", "", ""],
        ["B1", "主背景板项目 logo 位（同钻石级位规格）+ 加注「晚宴冠名战略合作伙伴」身份标识", "1 处（设计稿以甲方主背景板模板为准）", "甲方", "10,000", "1", "□ 已确认"],
        ["B2", "议程手册整版广告 + 项目折页夹页", "整版 A4 210×285mm，出血 3mm，CMYK 300 dpi", "甲方设计 / 印刷单位", "6,000", "1", "□ 已确认"],
        ["B3", "主会场宣传片轮播（嘉宾入场及中场休息时段）", "16:9 1920×1080，≤ 60 秒", "甲方", "4,000", "1", "□ 已确认"],
        ["B4", "500 份手拎袋夹页（项目折页 + 285/310 户型图，含印刷与装袋）", "三折页 297×210mm + 户型单页 × 500 套", "印刷单位 / 活动公司装袋", "20", "500", "□ 已确认"],
        ["C", "现场专属展位（小计：￥12,000）", "", "", "", "", ""],
        ["C1", "电梯口品牌展位（场地租赁 + 桌台轻包装）", "1 处（销售、易拉宝、KT 板、户型物料由乙方提供）", "甲方场地 / 活动公司基础包装", "6,000", "1", "□ 已确认"],
        ["C2", "展场内品牌展位（场地租赁 + 洽谈圆桌轻包装）", "1 处（销售、易拉宝、KT 板、户型物料由乙方提供）", "甲方场地 / 活动公司基础包装", "6,000", "1", "□ 已确认"],
        ["D", "会后口播与引导（小计：￥2,000）", "", "", "", "", ""],
        ["D1", "论坛 / 颁奖结束后主持人鸣谢口播 + 现场动线引导", "1 次主口播 + 现场引导员 1 名", "甲方", "2,000", "1", "□ 已确认"],
        ["E", "宣发与回执（小计：￥6,000）", "", "", "", "", ""],
        ["E1", "朋友圈九宫格（至少 3 张含项目 logo / KV）", "9 张 1080×1080 PNG", "甲方媒体组", "1,500", "1", "□ 已确认"],
        ["E2", "回顾视频片头 / 片尾项目鸣谢（含项目 logo 露出）", "1 条 1920×1080 mp4，鸣谢露出 ≤ 3 秒", "甲方", "2,500", "1", "□ 已确认"],
        ["E3", "执行回执（现场图片 + 嘉宾合影 + 1 份执行报告）", "大会结束后 7 个自然日内出具", "甲方", "2,000", "1", "□ 已确认"],
    ]
    table = doc.add_table(rows=1 + len(rows) + 1, cols=7)
    table.style = "Table Grid"
    table.autofit = False
    col_widths_cm = [1.2, 5.6, 4.2, 2.8, 1.6, 1.0, 1.9]
    for i, w in enumerate(col_widths_cm):
        for row in table.rows:
            row.cells[i].width = Cm(w)
    headers = ["序号", "服务项 / 物料", "规格 / 数量", "责任方", "单价(元)", "数量", "双方确认"]
    for i, h in enumerate(headers):
        set_cell_text(table.rows[0].cells[i], h, bold=True,
                      color=RGBColor(0xFF, 0xFF, 0xFF), size=10.5, align_center=True)
        shade_cell(table.rows[0].cells[i], "006B3F")
    for r, row in enumerate(rows):
        cells = table.rows[r + 1].cells
        is_category = (row[2] == "")
        if is_category:
            set_cell_text(cells[0], row[0], bold=True, size=10.5, align_center=True)
            set_cell_text(cells[1], row[1], bold=True, size=10.5)
            for ci in range(2, 7):
                set_cell_text(cells[ci], "", size=10.5)
            for ci in range(7):
                shade_cell(cells[ci], "E8F1EC")
        else:
            set_cell_text(cells[0], row[0], bold=True, size=10, align_center=True)
            set_cell_text(cells[1], row[1], size=10)
            set_cell_text(cells[2], row[2], size=10)
            set_cell_text(cells[3], row[3], size=10)
            set_cell_text(cells[4], row[4], size=10, align_center=True)
            set_cell_text(cells[5], row[5], size=10, align_center=True)
            set_cell_text(cells[6], row[6], size=10, align_center=True)
    total_cells = table.rows[-1].cells
    set_cell_text(total_cells[0], "合计", bold=True, color=RGBColor(0xFF, 0xFF, 0xFF), size=11, align_center=True)
    set_cell_text(total_cells[1], "人民币壹拾万元整（含税）", bold=True, color=RGBColor(0xFF, 0xFF, 0xFF), size=11)
    set_cell_text(total_cells[2], "—", color=RGBColor(0xFF, 0xFF, 0xFF), align_center=True)
    set_cell_text(total_cells[3], "—", color=RGBColor(0xFF, 0xFF, 0xFF), align_center=True)
    set_cell_text(total_cells[4], "￥100,000.00", bold=True, color=RGBColor(0xFF, 0xFF, 0xFF), size=11, align_center=True)
    set_cell_text(total_cells[5], "—", color=RGBColor(0xFF, 0xFF, 0xFF), align_center=True)
    set_cell_text(total_cells[6], "—", color=RGBColor(0xFF, 0xFF, 0xFF), align_center=True)
    for ci in range(7):
        shade_cell(total_cells[ci], "006B3F")
    blank(doc)

    para(doc, "说明：", bold=True, first_indent=False, size=10.5)
    para(doc, "1. 本报价 100,000 元为含税总价；发票内容：会议服务费 / 赞助费，由甲方按本协议第 7.2 条开具增值税普通发票或专用发票。", size=10)
    para(doc, "2. 上述执行项均为甲方对乙方的可交付物（含场地、物料植入、口播、宣讲位置、宣发、回执），过程中由甲方逐项归档证据材料（现场照片、印刷成品照、屏幕截图、回执报告等），并随《赞助权益执行回执》一并提交乙方作为验收依据。", size=10)
    para(doc, "3. 经双方协商，下列原洽谈条款本次合作不纳入或已作调整：", size=10)
    para(doc, "（1）白皮书扉页联合署名：因白皮书排期/出版不确定，本次合作不纳入；", size=10, first_indent=False)
    para(doc, "（2）媒体通稿「不少于 5 家头部财经/地产媒体」「标题级或副标题级」硬性指标：甲方按合作媒体清单进行推送，实际刊发数量、位置与口径以媒体自主决定为准，本附表不作硬性数量与位置承诺；", size=10, first_indent=False)
    para(doc, "（3）双校长三角校友产业联盟战略合作伙伴永久入册 + 牌匾交接：执行机制双方另议，本附表不纳入计价；", size=10, first_indent=False)
    para(doc, "（4）现场引导员由原 3 名调整为 1 名（合并至条款 D1），其余动线引导视嘉宾流量灵活补位；", size=10, first_indent=False)
    para(doc, "（5）同行业排他承诺范围限定为「晚宴冠名战略合作伙伴」身份及「主背景板钻石级 logo 位」，其他级别合作不在排他范围。", size=10, first_indent=False)
    para(doc, "4. 项目案场接驳（华为尊界 8–10 辆 + 考斯特补位）、项目案场接待（沙盘 + 样板间）、现场销售人员、展位升级包装等事项由乙方自办，不计入本报价 100,000 元，亦不构成甲方义务。", size=10)
    para(doc, "5. 本附表与本协议正文具有同等法律效力；本附表所列单价与数量为合作期内一次性、整体打包价格，单项不另议加价或退款，但因甲方原因未交付的，按本协议第 10.2 条处理。", size=10)
    blank(doc)
    ...
    for row in sig2.rows:
        row.cells[0].width = Cm(8.0)
        row.cells[1].width = Cm(8.0)
    set_cell_text(sig2.rows[0].cells[0], "甲方（盖章）：\n\n\n\n授权代表（签字）：______________________\n\n日期：______年______月______日", bold=True, size=11)
    set_cell_text(sig2.rows[0].cells[1], "乙方（盖章）：\n\n\n\n授权代表（签字）：______________________\n\n日期：______年______月______日", bold=True, size=11)
    set_cell_text(sig2.rows[1].cells[0], "（本附表一同时具备「报价单」与「执行清单」效力）", size=10, color=GREY, align_center=True)
    set_cell_text(sig2.rows[1].cells[1], "（与本附件协议正文具有同等法律效力）", size=10, color=GREY, align_center=True)
