...
title_block(ws, "夏春 · 长三角战略合作方案 — 配套测算与清单", "复旦大学住房政策研究中心 × 杨浦区科技企业联合会   |   2026年7月   |  与 PPT 方案配套使用", 3)
info = [
    ("目标", "作为“夏春财经智识”在长三角区域相关服务的独家 / 优先总代理。"),
    ("合作阶段", "长期战略协作 + 先行约 3 个月的敏感性适配过渡期（达标转正）。"),
    ("提报机构", "复旦大学住房政策研究中心；杨浦区科技企业联合会。"),
    ("差异化", "只做长三角区域增量，区隔于夏春现有新 / 老团队，避免存量与客户冲突。"),
    ("内容内核", "以夏春公开访谈 / 公众号 · 视频号观点与双方沟通纪要为依据（见“语录解析”）。"),
    ("", ""),
    ("工作表导航", ""),
    ...
    ("⑦ 风险与待办", "源自沟通纪要“待解决问题”的风险项、应对措施、责任方与优先级。"),
    ("⑧ KPI与转正门槛", "过渡期与长期的关键指标及由过渡转长期独家的达标条件。"),
    ("", ""),
...
title_block(ws, "夏春先生核心观点（语录）解析", "摘自公开访谈 / 公众号 · 视频号 + 双方沟通纪要，提炼为长三角合作切入点", 7)
hr = 3
...
quotes = [
    ...
    ("香港对虚拟资产的接受度……速度甚至比美国更快；现货 ETF 允许实物兑换，全球首创。", "访谈·香港/RWA", "香港虚拟资产 / RWA",
     "直接呼应纪要中的香港游学 RWA、中东资金窗口期。", "我方作长三角组团总入口，承接“半天讲座 + 半天参访”香港游学。", "高"),
    ...
    ("（纪要）香港游学需夏春博士全程参与；方案由夏春主导决策。", "沟通纪要", "IP 稀缺 / 减负",
     "夏春个人时间是核心稀缺资源，须为其减负。", "我方承接全部执行、排期与履约，录播 / 图文复用降低时间投入。", "高"),
    ("（纪要）依托复旦 / 科委背书突破企业制度限制，落地头部科技企业参访。", "沟通纪要", "背书 / 科技参访",
     "我方双背书正是突破点，与纪要重点高度契合。", "以复旦 + 杨浦科委背书，落地 AI / 科技前沿头部企业参访。", "高"),
]
r = hr + 1
...
title_block(ws, "3 个月过渡期行动计划（敏感性适配 → 达标转正）", "逐月 / 逐周任务、责任方、交付物与里程碑；状态列可下拉更新", 7)
hr = 3
style_header(ws, hr, ["阶段", "周次", "关键任务", "责任方", "交付物", "里程碑", "状态"])
plan = [
    ("第1月\n启动共识", "W1", "对齐目标与边界，确认过渡期授权范围", "双方", "合作意向确认", "启动", "待开始"),
    ("第1月\n启动共识", "W2", "起草并会签合作备忘录（MOU）与区域授权草案", "我方主起草", "MOU + 授权草案", "★ 签署 MOU", "待开始"),
    ("第1月\n启动共识", "W3", "客户 / 会员画像盘点，明确客户归属与分润规则", "双方", "客户画像 & 归属规则", "", "待开始"),
    ...
    ("第3月\n复盘转正", "W9", "第 2 场活动落地（另类配置 / 家办专场）", "双方", "活动交付", "第2场", "待开始"),
    ("第3月\n复盘转正", "W10", "第 3 场活动落地并沉淀活动 SOP", "双方", "活动 SOP", "★ 第3场", "待开始"),
    ("第3月\n复盘转正", "W11", "财务复盘、客户满意度与数据分析", "我方", "复盘报告", "", "待开始"),
    ("第3月\n复盘转正", "W12", "对照 KPI 评估，签署长期区域独家总代理协议", "双方", "长期协议", "★ 转正", "待开始"),
]
r = hr + 1
phase_colors = {"第1月\n启动共识": TEAL, "第2月\n首战落地": GOLD, "第3月\n复盘转正": NAVY2}
for p in plan:
...
title_block(ws, "长三角落地场景与内容矩阵", "结合夏春观点与沟通纪要；客单价 / 毛利为区间预估，需按实际招商调整", 9)
hr = 3
style_header(ws, hr, ["序号", "活动类型", "主题（结合夏春观点）", "目标客群", "频次", "单场人数", "参考客单价", "预估毛利率", "依托资源"])
matrix = [
    ("科技企业参访", "AI / 科技前沿 · 七巨头视角", "科创企业家 / 投资人", "月 1–2 场", "20–40", "0.5–3 万/人", "60%–90%", "杨浦科委 + 复旦背书"),
...
title_block(ws, "差异化定位对照：区别于夏春新 / 老团队", "只做长三角区域增量，边界清晰、互补不重叠（呼应纪要“主导权 / 客户归属”）", 4)
hr = 3
...
title_block(ws, "关键问题、风险与待办清单", "多数源自双方沟通纪要“待解决问题 / 待办事项”，过渡期内逐项落定并写入 MOU", 8)
hr = 3
style_header(ws, hr, ["序号", "事项", "类别", "来源", "应对措施", "责任方", "优先级", "状态"])
risks = [
    ("主导权与客户归属界定", "商务", "纪要·待解决", "明确长三角新增客户归属与交叉销售权益划分，写入 MOU。", "双方", "高", "待开始"),
    ("夏春档期与精力稀缺", "资源", "纪要·待解决", "我方承接执行与批量排期；录播 / 图文复用，关键场次才需本人。", "我方", "高", "待开始"),
    ("与新 / 老团队边界重叠", "商务", "纪要·待解决", "只做区域增量，签互不重叠 / 不竞争条款，定期同步避免撞单。", "双方", "高", "待开始"),
    ("异地执行精力有限", "运营", "纪要·待解决", "由我方长三角本地团队承接落地，夏春侧远程支持。", "我方", "中", "待开始"),
    ("合规与免责", "合规", "夏春惯例", "金融观点仅供参考、不构成投资建议；内容与产品销售分离。", "双方", "高", "待开始"),
    ("提供参访 / 分润 / 深圳方案案例", "交付", "纪要·待办(胡)", "沟通中由对方团队提供既有案例参考，我方据此做长三角适配。", "对方团队", "中", "待开始"),
    ("香港游学完整方案", "交付", "纪要·待办", "我方主导长三角组团方案，夏春确认讲座与档期。", "我方", "高", "待开始"),
    ("CRM / 内容（出书）方案", "交付", "纪要·待办", "我方搭建区域 CRM 与私域，内容出版按长期规划推进。", "我方", "中", "待开始"),
    ("形成合作备忘录（MOU）", "商务", "纪要·待办", "过渡期第 1 月内会签 MOU 与区域授权草案。", "双方", "高", "待开始"),
    ("建立微信群同步进度", "协作", "纪要·待办", "组建联合工作群，按周同步进度、加微对接细节。", "双方", "中", "待开始"),
]
r = hr + 1
...
# Sheet 8 — KPI 与转正门槛
# =====================================================
ws = wb.create_sheet("KPI与转正门槛")
ws.sheet_view.showGridLines = False
set_widths(ws, [5, 26, 30, 30, 18])
title_block(ws, "KPI 与由过渡转长期独家的门槛", "过渡期达标即触发长期区域独家总代理；目标为建议值，签约前双方确认", 5)
hr = 3
style_header(ws, hr, ["序号", "指标", "过渡期目标（约3个月）", "长期目标（年度）", "衡量方式"])
kpis = [
...
gate = ws.cell(row=r + 1, column=1,
               value="转正门槛：过渡期同时满足“活动场次 ≥ 3、营收 ≥ 60 万、毛利率 ≥ 60%、满意度 ≥ 4.5”，"
                     "即由过渡期自动转为长期区域独家总代理并会签正式协议。")
ws.merge_cells(start_row=r + 1, start_column=1, end_row=r + 2, end_column=5)
