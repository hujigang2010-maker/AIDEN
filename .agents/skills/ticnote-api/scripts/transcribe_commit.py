#!/usr/bin/env python3
"""提交转写任务（POST /api/v1/task/transcribe/commit）"""
import argparse
import json
import os
import sys
...
def transcribe_commit(base_url: str, token: str, file_id: int,
                      language: str = "zh", model: str = "",
                      has_speakers: bool = False, detail_level: str = "",
                      template: str = "", template_customize: str = "") -> dict:
    """POST /api/v1/task/transcribe/commit"""
    url = f"{base_url}/api/v1/task/transcribe/commit"
...
def main():
    parser = argparse.ArgumentParser(description="提交转写任务")
    ...
    parser.add_argument("--language", default="zh", help="语言代码，如 zh、en（默认 zh）")
    parser.add_argument("--model", default="", help="转写模型")
    ...
    parser.add_argument("--template", default="", help="录音笔模板")
    ...
    result = transcribe_commit(
        base_url, args.token, args.file_id,
        language=args.language, model=args.model,
        has_speakers=args.has_speakers, detail_level=args.detail_level,
