# -*- coding: utf-8 -*-
"""创智汇年度活动运营方案 · 最终定稿（多版合并 · 紫金主题）

优胜劣汰合并自：
- 年度运营方案PDF/融合PPT（结构骨架）
- 活动专项修订版（≤30人/带客销售/付款唯一）
- 双会资源融合版（定义核+WAIC/CJ导入）
- Excel0804（场次与口径定稿）
剔除：关联度核验表、千人级大场承诺、东莞/汕头/扬州潮玩场、租赁对赌主叙事、旧版冲突口径
视觉：皇家紫金；最新时点：2026.08（WAIC已闭幕、ChinaJoy刚结束，进入承接期）
"""
import os, shutil
...
def footer(s, page, total):
    text(s, ML, Inches(7.1), Inches(9.5), Inches(0.25),
         "上海创智汇 × 同浦汇 · 年度活动运营方案 · 最终定稿", size=11, color=SOFT)
...
def build():
    ...
    # 1 cover
    s = slide(prs)
    text(s, ML, Inches(1.45), Inches(12), Inches(0.3), "上海创智汇 × 同浦汇｜呈报园区　·　最终定稿", size=14, color=ACC, bold=True)
    text(s, ML, Inches(1.9), Inches(12), Inches(0.6), "创智汇年度活动运营方案", size=36, color=INK, bold=True)
    text(s, ML, Inches(2.6), Inches(12), Inches(0.35), "30场定稿 · 定义核与双会资源导入 · 2026.08—2027.07", size=16, color=GOLD, bold=True)
    text(s, ML, Inches(3.1), Inches(12), Inches(0.35),
         "AI+数字内容无界共创港　｜　WAIC已闭幕 · ChinaJoy刚结束 · 进入承接期", size=14, color=MUT)
    card(s, ML, Inches(3.8), Inches(5.8), Inches(2.9), "本册定稿要点", [
        "紫金主题视觉（多版合并后统一）",
        "每场≤30人 · 负责人≤30%",
        "同浦汇带客 / 园区销售",
        "WAIC→3F · ChinaJoy→5F",
    ...
    card(s, Inches(7.2), Inches(3.8), Inches(5.5), Inches(2.9), "边界", [
        "只取双会招商贴近切片，不堆全量名单",
        "出海另议另计价；领事到访/挂牌另计价",
        "不做租金租赁对赌与必要性要求",
        "不承诺必带外资企业；弱化关联度核验表",
    ...
    # 2 merge principles
    s = slide(prs); header(s, "00 · 合并说明", "多版材料优胜劣汰 → 一份定稿",
                           "合并自年度方案 / 活动专项修订 / 双会融合 / Excel0804")
    table(s, Inches(1.4), [
        ["保留（优）", "删除或降权（劣）"],
        ["年度方案章节骨架：背景→日历→主题→执行→转化", "把全量展商名录粘进PPT"],
        ["修订口径：≤30人、负责人≤30%、带客/销售", "关联度100%+《关联度核验表》交付"],
        ["双会定义核 + WAIC/CJ招商向切片", "千人级开放日/嘉年华承诺"],
        ["ChinaJoy主题替换东莞/汕头/扬州潮玩场", "租赁对赌、佣金对赌作活动主叙事"],
        ["付款唯一：50%+4×10%+10%", "出海/领事塞进年度包、必带外企承诺"],
        ["紫金主题统一视觉", "米白模板页与多套冲突口径并存"],
    ], [0.5, 0.5], sizes=[12, 12], rh=0.5)
    pg(s)

    # 3 toc
    s = slide(prs); header(s, "目录", "本方案结构（定稿）")
    table(s, Inches(1.45), [
        ["章节", "内容"],
        ["01 背景与总体思路", "最新时点、双会如何导入、做成什么样"],
        ["02 年度日历与运营节奏", "30场排期 + 四阶段 + 双会资源摘要"],
        ["03 六大主题活动详述", "A训练·B政策·C产业/CJ·D内容·E启动·F收官"],
        ["04 单场执行标准", "筹备→看场→建群→回访"],
        ["05 转化·分工·收费·案例", "带客/销售、付款、往期案例"],
        ["附录", "30场定义核与资源导入映射"],
    ], [0.28, 0.72], sizes=[14, 14], rh=0.48)
    pg(s)

    # 4 background
    s = slide(prs)
    header(s, "01 · 背景与判断", "双会之后，热度需要一个承接地",
           "最新时点：2026年8月 · WAIC已闭幕 · ChinaJoy刚结束 · 主题响应「AI+数字内容无界共创港」")
    card(s, ML, Inches(1.5), Inches(5.8), Inches(5.2), "判断", [
        "WAIC余热：议题/嘉宾/A-B线索需落到看场与洽谈",
        "ChinaJoy「与AI同游」：中小工作室/创作者/潮玩/具身可导入5F",
        "3F缺持续入孵节奏；5F缺持续内容人气",
        "因此：双会是资源库，30场是定义核",
        "启动期可选用北外滩一滴水做对外叙事夜，看场仍回创智汇",
    ], ACC)
    ...
    card(s, ML, Inches(4.25), Inches(11.9), Inches(2.4), "ChinaJoy → 5F（主题「与AI同游」）· 启动期可选北外滩一滴水叙事夜", [
        "可触达263优先 · 最高：中小游戏工作室 · 高：Vision Future具身 / 潮玩谷子 / 创作者经济",
        "中腰部选址灵活；不硬追大厂总部；看场转化仍回创智汇3F/5F",
    ], GOLD)
    pg(s)
    ...
    text(s, ML, Inches(3.3), Inches(12), Inches(0.4), "创智汇年度活动运营方案 · 最终定稿", size=18, color=GOLD, bold=True, align=PP_ALIGN.CENTER)
    ...
    table(s, Inches(1.45), [
        ...
        ["视觉", "皇家紫金主题（深紫渐变 + 蓝紫点缀 + 香槟金）· 最终定稿"],
    ], [0.28, 0.72], sizes=[12, 13], rh=0.38)
    pg(s)
    ...
    outs = [
        "创智汇年度活动运营方案-最终定稿.pptx",
        "chuangzhihui-annual-ops-plan-final.pptx",
    ...
    tmp = os.path.join(HERE, "_tmp_final.pptx")
    prs.save(tmp)
    for fn in outs:
    ...
    hdr(ws4, ["模块", "口径"])
    write(ws4, [
        ["定稿说明", "多版优胜劣汰合并；紫金主题；Excel0804场次为准"],
        ...
        ["最新时点", "2026.08：WAIC已闭幕、ChinaJoy刚结束，进入承接期"],
        ["出海/领事", "另议另计价；到访≠挂牌"],
        ["租金", "3.3元/㎡/天；不做对赌；免租期1–3个月面议"],
        ["付款", "签约后7日50%；四季各10%；年终10%"],
        ["剔除", "关联度核验表、千人级承诺、潮玩产业集群旧场、租赁对赌主叙事"],
    ], [18, 72], 34)

    for fn in ["创智汇年度活动运营方案-最终定稿.xlsx", "chuangzhihui-annual-ops-plan-final.xlsx",
               "创智汇年度活动运营方案-双会并入融合版.xlsx", "chuangzhihui-annual-ops-plan-dual-merge.xlsx",
               "创智汇年度活动运营方案-融合版.xlsx", "chuangzhihui-annual-ops-plan-fused.xlsx",
               "创智汇年度活动运营方案0804.xlsx", "创智汇30场活动专项交付-双会资源融合版.xlsx",
