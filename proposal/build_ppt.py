...
# ---------- 活动安排计划（年度活动日历） ----------
nxt(); s = slide(); header(s, "05", "EVENT CALENDAR", "活动安排计划 · 年度 30 场（表格化）"); footer(s, IDX)
table(s, ML, Inches(1.55), CW, [
    ["档位", "场次", "人数", "负责人占比", "孵化器关联度", "核心作用"],
    ["A 智能体/训练营", "8", "40–80", "35–45%", "邀约≥90%/到场≥80%", "OPC/Agent入驻转化"],
    ["B 政策/治理沙龙", "5", "40–80", "50–60%", "邀约≥90%/到场≥80%", "政策兑现+招商"],
    ["C 产业/科学对接", "5", "50–100", "55–65%", "≥85%", "成果/集群去化"],
    ["D 内容IP活动", "4", "80–300", "30–40%", "≥75%（流量场）", "5F引流+经营收入"],
    ["E WAIC联动大场", "4", "100–400", "40–55%", "≥80%", "大会流量+集中看场"],
    ["F 出海/AGI/收官", "4", "40–120", "50–60%", "≥90%", "出海撮合+年框签约"]],
    [0.2, 0.08, 0.12, 0.14, 0.22, 0.24], sizes=[11, 11, 11, 11, 11, 11], rh=Inches(0.48), head_rh=Inches(0.42), first_col_color=GOLD)
text(s, ML, Inches(5.85), Inches(11.6), Inches(0.55),
     "＊关联度不是口号式100%：允许约10–15%生态角色（政府/高校/投资人/媒体）；主体客群必须是可入驻画像企业。详表见《30场表格应答版》。",
     size=11, color=MUT)
text(s, ML, Inches(6.4), Inches(11.6), Inches(0.3),
     "＊商业锚点：办公 3.3 元/㎡/天 · 物业 13.8 元/㎡/月 · 待租办公室 8 间 · 佣金 2 个月。", size=11, color=GOLD)

# ---------- 活动价值论证（30 场 30 万怎么做/效果/收入） ----------
nxt(); s = slide(); header(s, "05", "EVENT VALUE", "30 场怎么做 · 价值 · 分工 · 收费（摘要）"); footer(s, IDX)
card(s, ML, Inches(1.55), Inches(3.9), Inches(2.55), "活动价值", accent=GOLD, items=[
    "导流 8 间待租看场与逼单",
    "年触达 900+ 精准客",
    "载体 KPI → 补贴 10–20 万/年",
    "媒体声量与品牌定调",
...
card(s, Inches(4.7), Inches(1.55), Inches(3.9), Inches(2.55), "交付物（每场）", accent=ACC, items=[
    "方案+议程+关联度核验表",
    "签到建档 / 意向分级表",
    "7 日回访纪要",
    "单场复盘（人数/关联度/转化）",
...
card(s, Inches(8.85), Inches(1.55), Inches(3.9), Inches(2.55), "分工一句话", accent=GREEN, items=[
    "我方：策划邀约执行回访",
    "贵方：场地物业名单看场交割",
    "联合会/服中心：政策协同",
    "最低支持：报备/导视/样板间/名单/会议室",
    "详表见 RACI",
], bsize=12)
rect(s, ML, Inches(4.35), CW, Inches(2.25), grad=[(0, "2A1E55", None), (100, "1E1542", None)], gang=120, line=GOLD, lw=1, radius=True)
rect(s, ML, Inches(4.35), Inches(0.07), Inches(2.25), fill=GOLD)
text(s, Inches(1.1), Inches(4.5), Inches(11), Inches(0.35), "收费模式", size=14, color=GOLD, bold=True)
bullets(s, Inches(1.1), Inches(4.95), Inches(5.6), Inches(1.4),
        ["活动打包：30 场 / 30 万元（策划执行+基础物料+主持）",
         "招商佣金：成交首年有效净租金 × 2 个月（首年不重复）",
         "办公 3.3 元/㎡/天；物业 13.8 元/㎡/月；待租 8 间优先活动转化"], size=12, gap=4, mark=GOLD)
bullets(s, Inches(6.9), Inches(4.95), Inches(5.5), Inches(1.4),
        ["门票/赞助默认归园区经营性收入（可协商分成）",
         "媒体投流约 10 万/年另计（可选）",
         "往期案例：杨数浦合规沙龙 / AI重塑DNA / 融见科创路演（含企业+议程+现场照）"], size=12, gap=4, mark=GOLD)
...
table(s, ML, Inches(1.72), CW, [
    ["收费项", "标准 / 口径", "支付方", "备注"],
    ["招商佣金（强化）", "成交后 2 个月租金（首年不重复）", "合作方", "市场标准，作招商板块主收费；招商不对赌"],
...
# ---------- 收费点价值论证总览 ----------
nxt(); s = slide(); header(s, "07", "VALUE JUSTIFICATION", "收费点价值论证 · 逻辑 / 价值 / 怎么做"); footer(s, IDX)
table(s, ML, Inches(1.68), CW, [
    ["收费点", "背后逻辑（凭什么）", "给中建的可衡量价值", "怎么做"],
    ["招商佣金", "牌照+政策+社群自带客流，110:1 漏斗", "去化提速、租金回收提前", "牌照锚定+政策礼包+社群/活动导流"],
    ["活动 30 场/30 万", "高频活动引流+完成载体 KPI", "转化租客、达标补贴 10–20 万/年、媒体声量", "6 类活动打包，投流引流→线下转化"],
    ["媒体 10 万", "官媒背书 + 自媒体投流", "项目背书、招商曝光 ≥150 万", "官媒 5 万 + 自媒体/投流 5 万"],
    ["政策申报", "挂牌科企服务中心+科企联+复旦系资质", "企业补贴到账、平台服务费/分成", "见「政策申报能力与清单」页"],
    ["企业服务", "联合公司承接注册/申报/知产/财税", "企业黏性、第二收入曲线", "联合公司经营分成"],
    ["出海撮合", "领事/大湾区/高校/国际英才资源", "海外订单、租金反哺、合资收益", "见出海板块（全链条+领事路径）"],
    ["国际会客厅", "领事资源合作方 + 高端背书", "外资对接、高端招商、≥80 万收益", "见「会客厅收益逻辑与机制」页"]],
    [0.14, 0.29, 0.31, 0.26], sizes=[10.5, 9.5, 9.5, 9.5], rh=Inches(0.52), head_rh=Inches(0.44), first_col_color=GOLD)
text(s, ML, Inches(6.55), Inches(11.6), Inches(0.3),
     "＊每个收费点＝背后逻辑（我们凭什么）+ 给中建的可衡量价值 + 具体怎么做；后续各页逐点展开。", size=9.5, color=SOFT)
...
kk = [("招商佣金", "2 月租金 · 成交计", GOLD), ("出海专项服务费", "原月费转化 · 出海协议内", ACC),
...
table(s, Inches(1.05), Inches(4.25), Inches(5.35), [
    ...
    ["招商板块", "佣金 2 月 + 活动 30 万 + 媒体 10 万（不对赌）"], ["出海板块", "出海专项服务费 + 撮合佣金（重收费）"]],
...
# ---------- 出海板块补充 · 国际英才（沪动营）+ 出海运营 ----------
nxt(); s = slide(); header(s, "08", "GLOBAL TALENT & OPS", "出海板块补充 · 国际英才 + 出海运营"); footer(s, IDX)
...
text(s, Inches(1.08), Inches(2.3), Inches(7.4), Inches(0.5),
     [("定位　", ACC, True), ("「沪联世界·智创未来」——财大/复旦/同济三校联合，服务逾万名在校留学生，培养懂中国·通国际的一带一路桥梁人才。", MUT, False)], size=11, space=1.15)
bullets(s, Inches(1.08), Inches(3.05), Inches(7.5), Inches(2.2), [
    ...
    "百千万工程（3 年）：孵化 100 跨境团队 / 促成 1000 人就业 / 培养 10000 复合人才；目标促成 10 亿跨境贸易、30 个海外品牌",
    "收入落点：训练营/创投大赛/跨境撮合佣金/企业结对 + 留学生政策项目"], size=10.5, gap=6, mark=ACC)
card(s, Inches(8.95), Inches(1.7), Inches(3.53), Inches(4.85), "出海运营（运营）", accent=ACC2, items=[
    "出海代运营：领英/谷歌/TikTok/短剧",
    "海外账号搭建与投放代运营",
    "出海活动运营 + 买家团/展销运营",
    "落地陪跑：连领事→部长→洽谈→落地",
    "团队：初期兼职 → 成熟全职约 3 人",
...
# ---------- 政策申报能力与可申报清单（凭什么能申 + 本项目可申哪些） ----------
nxt(); s = slide(); header(s, "08", "POLICY CAPABILITY", "政策申报 · 凭什么我们能申 + 本项目可申清单"); footer(s, IDX)
card(s, ML, Inches(1.7), Inches(5.72), Inches(2.5), "凭什么我们能申报（能力/资质）", accent=GOLD, bsize=10.5, items=[
    "挂牌科企服务中心 + 依托科企联，政务快通道",
    "拟入驻一网通办创新券平台，作服务机构核销",
    "复旦系（技术转移/珠海院/成果转化中心）+ 高校",
    "专业申报团队 + 过往经验（高企/专精特新）",
    "载体认定路径：载体/成果转化平台/OPC 社区"])
card(s, Inches(6.76), Inches(1.7), Inches(5.72), Inches(2.5), "本项目可申报清单（具体）", accent=ACC, bsize=10.5, items=[
    "企业侧：高企 20 万、专精特新 10/30 万、科小备案",
    "企业侧：AI 房租补贴 ≤100 万/年、算力 ≤50%、创新券 ≤30 万/年",
    "企业侧：微短剧专项（智能体/优秀剧/厂牌/出海）",
    "载体侧：成果转化平台运营 ≤100 万/年、服务业引导资金 ≤300 万",
    "载体侧：活动/载体补贴（满年 10–20 万）、载体认定"])
...
bullets(s, Inches(1.1), Inches(4.95), Inches(5.6), Inches(1.4), [
    "申报服务费：科小 0.3–2 万/家；高企/专精特新 2–8 万/家",
    "成功分成：补贴到账额 5%–15%（以兑现为准）",
    "载体运营补贴：认定+满年约 10–20 万/年（直接到平台）"], size=10.5, gap=5, mark=GREEN)
bullets(s, Inches(6.9), Inches(4.95), Inches(5.5), Inches(1.4), [
    "前提：先取得相应认定（载体/服务机构），摸排已列",
    "口径：补贴主体到企业/载体，平台按服务费+分成取酬",
    "先陪 3–5 家样板跑通，形成可复制申报流水"], size=10.5, gap=5, mark=GREEN)
...
# ---------- 国际会客厅收益逻辑与合作机制（≥80 万） ----------
nxt(); s = slide(); header(s, "08", "SALON LOGIC", "国际会客厅 · 收益逻辑与合作机制（≥80 万）"); footer(s, IDX)
...
text(s, Inches(1.1), Inches(1.68), Inches(11.2), Inches(1.0),
     [("合作机制　", GOLD, True), ("三方协同：平台（运营策划+活动执行+出海撮合）× 领事资源合作方（领事网络+空间）× 复旦政策研究中心/科企联（背书+获客）；以「国别/北欧会客厅」为载体，按 冠名 + 会籍 + 活动 + 出海对接 收费，收益按项目在三方间分润。", INK, False)],
...
table(s, ML, Inches(3.3), CW, [
    ...
    [0.26, 0.46, 0.28], sizes=[11, 10.5, 11], rh=Inches(0.5), head_rh=Inches(0.44), first_col_color=GOLD)
text(s, ML, Inches(6.5), Inches(11.6), Inches(0.3),
     "＊给中建的价值：以「国家为单位」的招商与外资对接入口、高端背书与外事资源；收益为保守测算，最终以合作方协议与执行为准。", size=9.5, color=SOFT)
...
q = [("Q1", "合作模式？", "轻资产运营（招商不对赌）：佣金 2 月 + 出海专项服务费 + 增值分成", GOLD),
     ("Q2", "报价档位？", "招商：佣金 2 月 + 活动 30 万 + 媒体 10 万 + 挂牌；办公3.3/物业13.8/待租8间", ACC),
