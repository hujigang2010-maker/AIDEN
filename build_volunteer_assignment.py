...
GROUPS = [
    {
        ...
        "sub_duties": [
            ("总指挥",          "全场最高决策；与主办/协办方对接；流程总控"),
            ("执行调度",        "对接 B/C/D/E/F/G 六组；时间节点把控；对讲机调度中枢"),
            ("文件秘书",        "议程手册、签到表、嘉宾名单、流程脚本、复盘记录"),
        ],
    },
    {
        "code": "B",
...
ASSIGNMENT = [
    ...
    ("A", "执行总监/副总策划",    "韦佳玉",            "—（总策划点名协助统筹）","23级校友、股权一级市场投融资、参与多届",
     "总策划会上直接点名协助统筹；多届经验+资源整合"),
    ("A", "文件秘书/科创联络",    "李振春（春子）",     "未明示",     "经院校友、科创业务、有会务/签到经验",
     "主动承担+科创资源（可对接科技展示赞助）"),

    # B 嘉宾接待组（6）—— 蒋珊调出至D / 吕志翔调出至E / 高辰辰调出至E / 彭常丽调出至F
    ("B", "组长",                 "王胜（20级）",      "B/F → B",   "第三次参加最资深；原 F 副组长升任 B 组长",
     "总策划指定升任：第三次参加经验最足，接替蒋珊带 B 组"),
    ...
    ("B", "VIP 1V1（核心搭子）",   "Winny",            "B/C/F均可 → B","空窗期时间最充裕、IP授权运营",
     "时间最充裕可全程紧贴VIP；总策划核心搭子之一"),
    ("B", "观众席引导",           "Angi 郭杰",         "B",         "国际贸易、首次",
     "采纳冯墨建议补观众席引导盲区，新人友好岗"),
    ("B", "动线引导",             "LV",               "B",         "—",
     "标准引导岗"),

    # C 会务执行组（5）—— 陈潇升组长、王珏降副但保留计时核心岗
    ...
    ("D", "AI 内容/新媒体（调入）","蒋珊",             "B → D",     "AI赛道、复旦人工智能俱乐部、跨校 C9 科创金融研究院",
     "AI 圈层资源最强；从 B 组调入做新媒体（金句卡/朋友圈/小红书）"),
    ("D", "直播+交响乐预热",      "皮尔德小号",        "未明示",     "互联网 10 年→城市更新、资源对接强",
     "主管直播；交响乐往届视频 T-7 起视频号每日预热"),
    ("D", "媒体接待+会后传播",    "马磊",             "未明示",     "AI Club 生态、跨界资源",
     "跨界资源+AI 圈层人脉，对接财经/科技媒体"),

    # E 技术支持组（3）—— 多 2 人调入：吕志翔 + 高辰辰
    ("E", "组长 / 技术对接",      "随圣博 Joshua-Sui","B/E/G → E", "复旦管院年会后勤经验",
     "唯一勾选 E，对接场馆 AV 外包+陈潇张江工厂设备"),
    ("E", "技术赞助/资源对接（调入）","吕志翔",        "B → E",     "12级、投融资、教育/医疗/AI",
     "投融资人脉，对接腾讯/华为技术方+协调技术赞助资源"),
    ("E", "技术对接助理（调入）", "高辰辰",            "B → E（听安排）","同浦汇、首次",
     "明确说听安排；从 B 调入做技术执行助理（标准化对接）"),

    # F 后勤保障组（3）—— 王胜调走、刘严升副、彭常丽调入
    ("F", "组长 / 场地+物料",     "朱俊峰",            "F",         "19级、建筑+市场开发、复旦网球协会运营",
     "建筑专业+协会长期运营，最熟物料/场地落地"),
    ("F", "副组长 / 物料+伴手礼+赞助物料","刘严",     "F",         "自报到F、平日接活多",
     "F 组组员升副组长；专项对接腾讯/华为赞助物料入场（去年痛点）"),
    ("F", "茶歇+晚宴专员（调入）","彭常丽",            "B/F → F",   "金融、二次参与",
     "去年已熟悉流程，从 B 调入主管 500 人茶歇+晚宴对接"),

    # G 应急/机动组（3）
    ("G", "组长 / 安保+机动调配", "康诺斯 Conns",     "B/G → G",   "设计改造+碳资产、首次",
     "做机动组长可跨组顶人"),
    ("G", "副组长 / 法务+舆情口径","张卓 盈科",        "B/G → G",   "财税律师、二次参加",
     "律师处理舆情口径与突发法务最稳"),
    ("G", "急救/医疗主岗 ★",      "蔡萍",             "B/G → G",   "复旦经院培训负责人(退休)、★持有急救证",
     "★稀缺技能：自报有急救证；G 组急救/医疗专项岗"),
]
...
LIAISON_LIST = [
    # (问题类别, 找谁, 备用)
    ("场地/桌椅",               "F 组 朱俊峰",       "F 组 刘严"),
    ("灯光/舞台/音响（自有）",   "C 组 陈潇 Kelly",   "E 组 随圣博"),
    ("麦克风/PPT切换/网络/直播",  "E 组 随圣博",       "E 组 高辰辰"),
    ("技术赞助/腾讯华为技术对接", "E 组 吕志翔",       "胡继刚（总策划）"),
    ("酒店/嘉宾接送/休息室",    "B 组 王胜",         "韦佳玉（A 组）"),
    ("VIP 嘉宾临时需求",        "B 组对应 1V1 责任人","王胜（B 组长）"),
    ("观众席引导/秩序",          "B 组 Angi",        "B 组 LV / G 组 康诺斯"),
    ("茶歇/晚宴",               "F 组 彭常丽",       "F 组 朱俊峰"),
    ("赞助物料（腾讯/华为）",   "F 组 刘严",         "胡继刚"),
    ("摄影/摄像",               "D 组 张蒙",         "葛九明（D 组长）"),
    ("直播信号/新媒体即时发布",  "D 组 皮尔德小号",   "蒋珊（AI 内容）"),
    ("AI 圈层金句/新媒体素材",   "D 组 蒋珊",         "马磊"),
    ("媒体/记者接待",           "D 组 马磊",         "葛九明"),
    ("讲者 PPT/资料/换片",      "C 组 黄璐",         "陈潇 Kelly"),
    ("流程超时/嘉宾迟到",       "C 组 王珏（计时）",  "陈潇（C 组长）→ 韦佳玉"),
    ("颁奖物料/获奖嘉宾",       "C 组 徐胜博",       "王珏"),
    ("法务/舆情/突发声明",       "G 组 张卓",        "胡继刚"),
    ("医疗/急救 ★",             "G 组 蔡萍 ★",       "G 组 康诺斯"),
    ("跨组人手不足/机动调配",    "G 组 康诺斯",      "韦佳玉"),
    ("交响乐团对接",             "F 组 朱俊峰",      "韦佳玉（去年负责）"),
    ("晚宴席位/敬酒/现场对接",   "F 组 彭常丽",      "B 组 王胜"),
    ("无法解决/重大决策",        "韦佳玉 → 胡继刚",  "—"),
]
...
BACKUP_ROLES = [
    # (组, A 角组长, B 角备份, 备注)
    ("A 统筹",   "胡继刚（总策划）", "韦佳玉",        "总策划缺席由韦佳玉全权代行"),
    ("B 接待",   "王胜",          "朱铭喆",        "A 角第三次参加最资深；B 角港校联络人"),
    ("C 会务",   "陈潇 Kelly",    "王珏",          "A 角 20 年策划经验+设备；B 角中信证券计时主控"),
    ("D 媒体",   "葛九明",        "张蒙 AEGISTAR", "A 角总策划指定；B 角自媒体+摄影核心岗"),
    ("E 技术",   "随圣博",        "吕志翔",        "B 角投融资人脉对接技术赞助方"),
    ("F 后勤",   "朱俊峰",        "刘严",          "A 角建筑+协会运营；B 角对接赞助物料"),
    ("G 应急",   "康诺斯",        "张卓",          "B 角律师可应对突发；蔡萍持急救证常驻医疗岗"),
]
...
def build_ppt(path):
    ...
    # ============ 本轮人员调整说明 ============
    s = prs.slides.add_slide(blank)
    slide_title_bar(s, "本轮人员调整说明（总策划 5/14 调整）",
                    "原则：人数平均分配 / 嘉宾接待组瘦身 / 关键岗位组长就位")

    # 左侧：调整动作
    add_rect(s, Inches(0.55), Inches(1.4), Inches(6.0), Inches(0.5),
             "C00000", text="本轮 10 项调整", text_size=14, text_bold=True)
    moves = [
        ("人员", "+ 葛九明 → D 组组长（新增）"),
        ("人员", "蒋珊：B 组 → D 组（AI 圈层做新媒体）"),
        ("人员", "吕志翔：B 组 → E 组（投融资对接技术赞助）"),
        ("人员", "高辰辰：B 组 → E 组（听安排，技术对接助理）"),
        ("人员", "彭常丽：B 组 → F 组（茶歇/晚宴专员）"),
        ("组长", "王胜：F 副组长 → B 组组长（最资深）"),
        ("组长", "陈潇 Kelly：C 副组长 → C 组组长（升任）"),
        ("组长", "王珏：C 组长 → C 副组长（保留计时核心岗）"),
        ("组长", "张蒙：D 组长 → D 副组长（保留摄影核心岗）"),
        ("组长", "刘严：F 组员 → F 副组长（升）"),
    ]
    row_h = 0.5
    for i, (k, v) in enumerate(moves):
        y = Inches(2.0 + i*row_h)
        color = "C00000" if k == "组长" else "ED7D31"
        add_rect(s, Inches(0.55), y, Inches(0.9), Inches(row_h-0.05),
                 color, text=k, text_size=10, text_bold=True)
        add_rect(s, Inches(1.5), y, Inches(5.05), Inches(row_h-0.05),
