...
覆盖杨浦区 **50 栋**商业写字楼与工业/产业楼宇（写字楼 26 / 产业园 17 / 混合 7），共 91 条入驻企业记录，横跨五角场、大创智、新江湾城、杨浦滨江（长海路/平凉路）、大连路/杨浦外滩、长阳路、控江路、江浦路、四平路、殷行、大桥、定海路、军工路等街镇。

> 数据综合自杨浦区政府公开文件、园区公示信息与主流商办平台公开行情整理，仅供参考，可在 `src/data/buildings.ts` 中替换为内部业务数据。

### 简版 / 详细版表格导出

- 页面「楼宇数据」标签内提供 **导出简版**（每栋楼宇一行）与 **导出详细版**（每个入驻企业一行）按钮，按当前筛选条件导出。
- 命令行一键生成：`npm run release`，产物见 `release/` 目录。
